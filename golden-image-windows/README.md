# Golden Image Windows — Terraform Module

This directory contains the Terraform module for the Windows Server 2022 Golden Image Pipeline using EC2 Image Builder.
...
├── scripts/
│   ├── bootstrap-sandbox.sh     # One-time sandbox bootstrap (creates state bucket, KMS key, tfvars, runs first apply, pushes secrets)
│   └── destroybootstrap-sandbox.sh # Sandbox teardown (terraform destroy, KMS purge, removes GitHub secrets, cleans local files)
├── provider.tf                  # AWS provider config, required_version
├── variables.tf                 # Input variables (subnet, SG, KMS, accounts, GitHub OIDC, etc.)
├── oidc-variables.tf            # OIDC-specific variables
...
## Known Gaps / Future Work (from root README)
