...
#
# Automates the one-time bootstrap of a sandbox account for the golden
# image pipeline. After this script finishes, all subsequent apply/
# destroy operations are expected to run from CI (GitHub Actions via
# OIDC), not from this laptop.
#
# Phases (in execution order):
#   0. Confirm AWS identity
#   1. Provision S3 remote-state bucket (versioned, encrypted, private)
#      and write backend.hcl for partial backend config
...
# ---------------------------------------------------------------------------
# Step 1: Provision the S3 remote-state bucket + write backend.hcl
# ---------------------------------------------------------------------------
# The Terraform state bucket is created OUT-OF-BAND here because it
# cannot be managed by the same Terraform configuration that uses it
# as a backend (classic chicken-and-egg).
#
# After this step the bucket has:
#   - versioning enabled (required for safe state recovery)
#   - default SSE-S3 encryption (AES256)
#   - all public access blocked
#   - lifecycle rule to abort incomplete multipart uploads after 7 days
#
# The same name is written into backend.hcl so that `terraform init
# -backend-config=backend.hcl` resolves the partial backend block in
# provider.tf without needing to edit any .tf file.
#
# Idempotent: if the bucket already exists (e.g. a previous bootstrap
# run), we skip creation and just re-read its properties.
# ---------------------------------------------------------------------------
echo "--- Step 1: Provisioning S3 remote-state bucket ---"

STATE_BUCKET="${NAME_PREFIX}-tfstate-${ACCOUNT_ID}-${AWS_REGION}"

if aws s3api head-bucket --bucket "$STATE_BUCKET" 2>/dev/null; then
  echo "Bucket ${STATE_BUCKET} already exists — reusing it."
else
  echo "Creating bucket ${STATE_BUCKET} in ${AWS_REGION}..."

  # CreateBucket parameters differ between AWS partitions (us-east-1
  # does NOT accept a LocationConstraint; everywhere else does).
  ...
  aws s3api put-bucket-tagging \
    --bucket "$STATE_BUCKET" \
    --tagging "TagSet=[
      {Key=Project,Value=golden-image-lab},
...
STATE_KEY="golden-image-windows/terraform.tfstate"

echo ""
echo "State bucket : ${STATE_BUCKET}"
echo "State key    : ${STATE_KEY}"
echo "State region : ${STATE_BUCKET_REGION}"
echo ""

# Write backend.hcl — this file is consumed by `terraform init -backend-config=backend.hcl`.
# It overrides only the values that vary per environment (bucket name, region, key);
# the rest of the backend block (encrypt, use_lockfile, etc.) lives in provider.tf.
cat > backend.hcl <<EOF
# Auto-generated by bootstrap-sandbox.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ).
# Do not edit by hand — re-run bootstrap-sandbox.sh to regenerate.
# This file is gitignored; commit nothing equivalent to it.

bucket = "${STATE_BUCKET}"
region = "${STATE_BUCKET_REGION}"
key    = "${STATE_KEY}"
EOF
echo "Wrote backend.hcl (partial backend config)."
echo ""
...
# --- Terraform remote-state backend (matches backend.hcl) ---
...
# `terraform init` with the partial backend config from backend.hcl.
# -migrate-state moves any existing local state into the S3 bucket so
# this script is safe to run on top of a previously locally-applied
# configuration.
echo "Initialising Terraform with remote state backend (S3 + native locking)..."
terraform init -input=false -backend-config=backend.hcl -migrate-state
...
push_secrets_via_gh() {
  ...
  echo "  TF_STATE_REGION                    = ${STATE_BUCKET_REGION}"
  ...
  set_one_secret "TF_STATE_BUCKET"                    "$STATE_BUCKET" || rc=1
  set_one_secret "TF_STATE_REGION"                    "$STATE_BUCKET_REGION" || rc=1
  set_one_secret "TF_STATE_KEY"                       "$STATE_KEY" || rc=1
...
echo "  5. Subsequent apply/destroy operations should be driven from GitHub Actions:"
echo "       .github/workflows/golden-image-terraform.yml         (plan on PR, apply on push to main)"
echo "       .github/workflows/golden-image-terraform-destroy.yml (workflow_dispatch only)"
echo "     Local apply/destroy is no longer the source of truth — the state lives in S3 and is"
echo "     locked there via native .tflock files."
echo ""
